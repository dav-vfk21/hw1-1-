...
<body>
    ...
    <main>
        <section class="mostra_generi">
          <div class="generi_serie_film">
             <h1>Generi<span>/</span>Storico</h1>
        <?php  
             echo "<div class='scelta_generi'>";
            echo  "<div class='flex_scelta_generi'>";
            if ($tipo == 'serie') {
               echo "<a id='series' class='active' href='storico.php?tipo=serie'>Serie</a>";
               echo "<a id='films' href='storico.php?tipo=film'>Film</a>";
            } elseif ($tipo == 'film') {
               echo "<a id='series' href='storico.php?tipo=serie'>Serie</a>";
               echo "<a id='films' class='active' href='storico.php?tipo=film'>Film</a>";
            } else {
               echo "<a id='series' href='storico.php?tipo=serie'>Serie</a>";
               echo "<a id='films' href='storico.php?tipo=film'>Film</a>";
            }
  ...
  <?php
    ...
    if ($tipo === 'film') {
 if (isset($generi_film['storia'])) {
        $genere_id = $generi_film['storia'];
    }
        } else {
     if (isset($generi_serie['storia'])) {
        $genere_id = $generi_serie['storia'];
       } 
      }
      if ($tipo == 'serie' || $tipo == '') {
...
if ($page > 1) {
    $prev = $page - 1;
    echo "<a href='storico.php?tipo=$tipo&page=$prev' class='pagina_destra'><img src='precedente.png'>Precedente</a>";
}

$next = $page + 1;
echo "<a href='storico.php?tipo=$tipo&page=$next' class='pagina_sinistra'>Successiva <img src = 'successivo.png'</a>";
echo "</div>";
    }
} else {
