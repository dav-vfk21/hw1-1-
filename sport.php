    ...
    <main>
        <section class="mostra_generi">
          <div class="generi_serie_film">
             <h1>Generi<span>/</span>Sport</h1>
              <?php  
             echo "<div class='scelta_generi'>";
            echo  "<div class='flex_scelta_generi'>";
            if ($tipo == 'serie' || $tipo == '') {
               echo "<a id='series' class='active' href='sport.php?tipo=serie'>Serie</a>";
               echo "<a id='films' href='sport.php?tipo=film'>Film</a>";
            } elseif ($tipo == 'film') {
               echo "<a id='series' href='sport.php?tipo=serie'>Serie</a>";
               echo "<a id='films' class='active' href='sport.php?tipo=film'>Film</a>";
            } else {
               echo "<a id='series' href='sport.php?tipo=serie'>Serie</a>";
               echo "<a id='films' href='sport.php?tipo=film'>Film</a>";
            }
     ...
     $conta_poster = 0;
     if ($tipo === 'film') {
    if (isset($generi_film['sport'])) {
        $genere_id = $generi_film['sport'];
    }
} else {
    if (isset($generi_serie['sport'])) {
        $genere_id = $generi_serie['sport'];
    } 
}
      if ($tipo == 'serie' || $tipo == '') {
        $url = "https://api.themoviedb.org/3/discover/tv?api_key=$api_key&with_keywords=$genere_id&page=$page&language=it-IT";
          } elseif ($tipo == 'film') {
        $url = "https://api.themoviedb.org/3/discover/movie?api_key=$api_key&with_keywords=$genere_id&page=$page&language=it-IT";
      } else {
        $url = '';
   }
...
if ($page > 1) {
    $prev = $page - 1;
    echo "<a href='sport.php?tipo=$tipo&page=$prev' class='pagina_destra'><img src='precedente.png'>Precedente</a>";
}
    }
$next = $page + 1;
echo "<a href='sport.php?tipo=$tipo&page=$next' class='pagina_sinistra'>Successiva <img src = 'successivo.png'</a>";
echo "</div>";
