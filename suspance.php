...
<body>
    ...
    <main>
        <section class="mostra_generi">
          <div class="generi_serie_film">
             <h1>Generi<span>/</span>Suspance</h1>
        <?php  
             echo "<div class='scelta_generi'>";
            echo  "<div class='flex_scelta_generi'>";
            if ($tipo == 'serie') {
               echo "<a id='series' class='active' href='suspance.php?tipo=serie'>Serie</a>";
               echo "<a id='films' href='suspance.php?tipo=film'>Film</a>";
            } elseif ($tipo == 'film') {
               echo "<a id='series' href='suspance.php?tipo=serie'>Serie</a>";
               echo "<a id='films' class='active' href='suspance.php?tipo=film'>Film</a>";
            } else {
               echo "<a id='series' href='suspance.php?tipo=serie'>Serie</a>";
               echo "<a id='films' href='suspance.php?tipo=film'>Film</a>";
            }
  ...
  <?php
    ...
    if ($tipo === 'film') {
 if (isset($generi_film['suspance'])) {
        $genere_id = $generi_film['suspance'];
    }
        } else {
     if (isset($generi_serie['suspance'])) {
        $genere_id = $generi_serie['suspance'];
       } 
      }
      if ($tipo == 'serie' || $tipo == '') {
...
if ($page > 1) {
    $prev = $page - 1;
    echo "<a href='suspance.php?tipo=$tipo&page=$prev' class='pagina_destra'><img src='precedente.png'>Precedente</a>";
}

$next = $page + 1;
echo "<a href='suspance.php?tipo=$tipo&page=$next' class='pagina_sinistra'>Successiva <img src = 'successivo.png'</a>";
echo "</div>";
 }
} else {
